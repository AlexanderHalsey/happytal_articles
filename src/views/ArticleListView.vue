  ...
  export default defineComponent({
    ...
    methods: {
      async fetchData () {
        this.error = null
        this.loading = true
        const rawData = await fetch("https://cms-beta.happytal.com/articles");
        this.loading = false;
        if (rawData.ok) {
          const data = await rawData.json();
          this.articles = data;
          this.filtered_articles = data;
        } else {
          this.error = "we got an error !";
        }
      },
      onChange (val: string) {
        this.filter_value = val;
        this.filtered_articles = this.articles.filter((article: ArticleDto): boolean => {
...
<template>
  <main>
    ...
    <div v-if="articles.length > 0" class="scroll_container">
